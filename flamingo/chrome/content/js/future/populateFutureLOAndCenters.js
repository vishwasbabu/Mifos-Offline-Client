(function($){
    $.fn.populateFutureLOAndCenters = function(){
        var d = new Date();
        ...
        $.each(nobj.CollectionSheets, function(h, collectionSheet){
            ...
            $.each(collectionSheet.LoanOfficers, function(i, loanofficer){ //looping LO
                ...
                $.each(loanofficer.Centers, function(j, center){ //looping Center
                    ...
                    label.setAttribute("onclick", "$('#futureCenterCollectionsheet').populateCenterCollectionSheet({loanOfficerName:'" + loanofficer.name + "',date:'" + collectionSheet['@date'] + "',centerid : '" + center.centerId + "',idPrefix :" + "'future'" + "});");
                    row.appendChild(label);
